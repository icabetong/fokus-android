...
class ArchivedSubjectAdapter: BaseAdapter<SubjectPackage, ArchivedSubjectAdapter.ArchivedSubjectViewHolder>(SubjectPackage.DIFF_CALLBACK) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ArchivedSubjectViewHolder {
        val binding = LayoutItemArchivedSubjectBinding.inflate(LayoutInflater.from(parent.context),
            parent, false)
        return ArchivedSubjectViewHolder(binding.root)
    }
    ...
    class ArchivedSubjectViewHolder(itemView: View): BaseViewHolder(itemView) {
